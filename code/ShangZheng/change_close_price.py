...
def update_close_prices(holdings_path):
    ...
    # holdings_df = pd.read_csv(holdings_path, encoding='gbk')
    ...
    # 确保返回的 DataFrame 包含 'Code' 和 'NOW' 列（假设收盘价列名为 'NOW'）
    if 'Code' in price_df.columns and 'PRECLOSE' in price_df.columns:
        ...
        holdings_df['收盘价'] = [
            0 if code in ['204001', '131810'] else price_mapping.get(code, None) 
