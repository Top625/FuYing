...
def detect_encoding(file_path):
    encoding = codecs.open(file_path, "r").encoding

    # with open(file_path, 'rb') as f:
    #     # 读取前 1024 字节进行初步检测
    #     raw_data = f.read(1024*1024*1024)
    #     result = chardet.detect(raw_data)
    #     encoding = result['encoding']

    #     # 如果检测结果置信度较低，读取整个文件重新检测
...
def deal_part_data(today, account_file, position_file, output_file):
    ...
    with open(position_file, 'r', encoding=position_encoding) as f:
        ...
        for row in position_reader:
            ...
            if row[name_index] == account_name:
                ...
                print(market_value, current_shares, latest_price)
    ...
    if account_name == '中泰XTP实盘_26节点':
        account_name = '尊享2号'
    elif account_name == '富赢九章量化1号':
        account_name = '国信'
   ...
   # 获取申购赎回金额
    # subscription_redemption_path = r'C:\Users\Top\Desktop\ftp\申购赎回记录.csv'
...
if __name__ == "__main__":
    ...
    # today = '20250528'
    name = '山西证券'
    account_file = rf'c:\Users\Top\Desktop\{name}\{name}-Account-{today}.csv'
    position_file = rf'c:\Users\Top\Desktop\{name}\{name}-PositionStatics-{today}.csv'
    output_file = rf'c:\Users\Top\Desktop\{name}\{name}-part-result.csv'
